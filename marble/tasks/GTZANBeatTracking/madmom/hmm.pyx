...
"""
marble/tasks/GTZANBeatTracking/madmom/hmm.pyx
...
class HiddenMarkovModel(object):
    ...
    def reset(self, initial_distribution=None):
        ...
        if initial_distribution is None:
            ...
            # It's good practice to also validate the provided initial_distribution here,
            # similar to how Numba's reset does.
            initial_distribution = np.asarray(initial_distribution, dtype=float)
            if not np.allclose(initial_distribution.sum(), 1) or \
            initial_distribution.shape[0] != self.transition_model.num_states:
                raise ValueError("Provided initial_distribution for reset is invalid.")
            self._prev = initial_distribution.copy()
    ...
    def viterbi(self, observations):
        ...
        if num_observations == 0:
            ...
            # Numba's behavior for empty sequence is to return max(log(initial_distribution))
            # and an empty path. We replicate that here for consistency.
            if num_states == 0: # Should ideally not happen with a valid HMM
                return np.empty(0, dtype=np.uint32), -INFINITY # or some very small number / error
            # Ensure initial_distribution is not empty if num_states > 0
            if self.initial_distribution.shape[0] == 0 and num_states > 0: # Also unlikely for valid HMM
                 return np.empty(0, dtype=np.uint32), -INFINITY
            
            # Calculate np.log only if initial_distribution is not already log (it's not for self.initial_distribution)
            # Numba's kernel receives init_log_dist directly. Cython gets self.initial_distribution.
            log_initial_dist = np.log(self.initial_distribution)
            # Handle case where all log_initial_dist might be -inf (e.g. initial_distribution had zeros)
            if np.all(np.isneginf(log_initial_dist)):
                 max_log_init_prob = -INFINITY
            else:
                 max_log_init_prob = np.max(log_initial_dist[np.isfinite(log_initial_dist)]) if np.any(np.isfinite(log_initial_dist)) else -INFINITY

            return np.empty(0, dtype=np.uint32), max_log_init_prob
