...
import torch
import random
from marble.encoders.CLaMP3.config import *
try:
...
def unicode_to_ascii(text):
    """
    A dependency-free alternative to the unidecode library.
    Converts Unicode text to its closest ASCII representation.
    """
    # Normalizes the string to separate base characters from diacritics (accents)
    normalized_text = unicodedata.normalize('NFD', text)
    # Encodes to ASCII, ignoring non-ASCII characters (the diacritics),
    # and then decodes back to a string.
