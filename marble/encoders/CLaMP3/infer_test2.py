# marble/encoders/CLaMP3/infer_test2.py
import os
import json
import torch
import numpy as np
...
# === Step 1: Extract audio feature for blues.00000.wav ===
print("\nProcessing audio file: tests/blues.00000.wav")
...
print("Audio feature extracted and saved to", audio_feature_path)

# === Step 2: Define GTZAN genres and calculate similarities ===
gtzan_genres = ["blues", "classical", "country", "disco", "hiphop", "jazz", "metal", "pop", "reggae", "rock"]
similarities = {}
temp_text_path = "tests/temp_genre.txt"

# Load the saved audio feature
audio_feature = torch.from_numpy(np.load(audio_feature_path)).cuda()

print("\nCalculating similarities with GTZAN genres...")
for genre in tqdm(gtzan_genres):
    # Write genre name to a temporary file
    with open(temp_text_path, "w") as f:
        f.write(genre)
    
    # Extract text feature for the genre
    with torch.no_grad():
        text_feature = extract_feature_clamp3(temp_text_path, get_global=True).unsqueeze(0)
    ...
    # We use .sum() because the features are already normalized and have a batch size of 1
...
print("\n--- Similarity Results for 'blues.00000.wav' ---")
