...
from marble.encoders.DaSheng.train.models import AudioTransformerMAE_Encoder
...
def dasheng_base(pre_trained_folder=None, **model_kwargs):
    model_kwargs["embed_dim"] = 768
    model_kwargs["depth"] = 12
    model_kwargs["num_heads"] = 12
    return Dasheng.from_pretrained(
        pre_trained_folder or PRETRAINED_CHECKPOINTS["dasheng_base"], **model_kwargs
    )


def dasheng_06B(pre_trained_folder=None, **model_kwargs):
    model_kwargs["embed_dim"] = 1280
    model_kwargs["depth"] = 32
    model_kwargs["num_heads"] = 16
    return Dasheng.from_pretrained(
        pre_trained_folder or PRETRAINED_CHECKPOINTS["dasheng_06B"], **model_kwargs
    )


def dasheng_12B(pre_trained_folder=None, **model_kwargs):
    model_kwargs["embed_dim"] = 1536
    model_kwargs["depth"] = 40
    model_kwargs["num_heads"] = 24
    return Dasheng.from_pretrained(
        pre_trained_folder or PRETRAINED_CHECKPOINTS["dasheng_12B"], **model_kwargs
    )
