...
from marble.encoders.Xcodec.modules.seanet import SEANetEncoder, SEANetDecoder
from marble.encoders.Xcodec.quantization  import ResidualVectorQuantizer#,VectorQuantize
from transformers import  AutoModel
from transformers import AutoFeatureExtractor, WhisperModel
from marble.encoders.Xcodec.repcodec.modules.encoder import Encoder
from marble.encoders.Xcodec.repcodec.modules.decoder import Decoder
#descript-audio-codec/dac/model
import  marble.encoders.Xcodec.descriptaudiocodec.dac.model.dac  as dac2
...
class SoundStream(nn.Module):
    ...
    def __init__(
        ...
        if self.is_semantic:
            # self.semantic_model = AutoModel.from_pretrained("/scratch/buildlam/codeclm/Audio-Flan/Audio-Flan/extract_codec/SoundCodec/base_codec/xcodec/semantic_ckpts/hf_1_325000")
            self.semantic_model = AutoModel.from_pretrained(os.path.join(Path.home(), ".cache", "xcodec","semantic_ckpts/hf_1_325000"))
    ...
    def encode(self, x: torch.Tensor, target_bw: Optional[int] = None, mode='indices') -> torch.Tensor:
        # e = self.encoder(x)
        ...
        if mode == 'indices':
            ...
            return quantized
        elif mode == 'pre_vq_emb':
            return e
        else:
            raise ValueError(f"Unknown mode {mode} for SoundStream encode method.")
