...
class MuQMuLan(nn.Module, PyTorchModelHubMixin):
    ...
    def _to_obj(self, config):
        ...
        # 1) 如果用户直接传了 dataclass 实例，先转成 dict
        if is_dataclass(config):
            config = asdict(config)

        # 2) 如果是普通 dict，就保留
        if isinstance(config, dict):
            # 2.1) 先构造出一个空的“schema” DictConfig
            schema: DictConfig = OmegaConf.structured(MuQMuLanConfig)
            # 2.2) 把用户的 dict merge 进 schema
            config = OmegaConf.merge(schema, OmegaConf.create(config))
            return config

        # 3) 如果已经就是 DictConfig，直接用
    ...
    def _get_all_clips(self, audio):
        ...
        accum_length = 0
        delta = int(self.sr * self.clip_secs)
        audio_clips = []
