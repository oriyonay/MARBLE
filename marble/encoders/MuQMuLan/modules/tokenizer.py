...
# https://github.com/lucidrains/x-clip/blob/main/x_clip/tokenizer.py
...
@lru_cache()
def default_bpe():
    return os.path.join(os.path.dirname(os.path.abspath(__file__)), "vocab/bpe_simple_vocab_16e6.txt")
...
def basic_clean(text: str) -> str:
    # 1) Unicode 正规化（合并分解字符）
    text = unicodedata.normalize('NFC', text)
    # 2) 反两遍 HTML 实体（有时候会被双重转义）
    text = html.unescape(html.unescape(text))
    return text.strip()
